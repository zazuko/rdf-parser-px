...
class RdfBuilder extends Readable {
  constructor ({ baseIRI, columns = [], observer, nullValue }) {
    super({
      objectMode: true,
      read: () => {}
    })
  ...
  import (pxCube) {
    ...
    const shapeTerm = rdf.namedNode(new URL('shape', this.baseIRI).toString())
    const observationSetTerm = rdf.namedNode(new URL('observation/', this.baseIRI).toString())
    ...
    for (const observation of pxCube.observations()) {
      const observationTerm = rdf.namedNode(new URL(`observation/${observation.row}`, this.baseIRI).toString())
      ...
      observation.measures.forEach(measure => {
        ...
        this.push(rdf.quad(observationTerm, column.property, column.value(measure.value, { nullValue: this.nullValue })))
      })
