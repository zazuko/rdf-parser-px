...
class Metadata {
  ...
  addProperty ({ shapePtr, path, type, datatype, nullValues, measure, valueKeys, values = {}, min, max, labels = {} } = {}) {
    shapePtr.addOut(ns.sh.property, propertyPtr => {
      ...
      if (datatype) {
        if (nullValues) {
          propertyPtr.addList(ns.sh.or, [
            propertyPtr.blankNode().addOut(ns.sh.datatype, datatype),
            propertyPtr.blankNode().addOut(ns.sh.datatype, ns.cube.Undefined)
          ])
        } else {
  ...
  addColumnProperty ({ shapePtr, columns, title, values = {}, labels = {} } = {}) {
    ...
    const nullValues = column.nullValues
    const measure = column.measure
    const valueKeys = column.values
    const min = column.min
    const max = column.max

    this.addProperty({ shapePtr, path, type, datatype, nullValues, measure, valueKeys, min, max, values, labels })
  }
